...
if not exist "%MSVC_LIB%" (
    echo Missing MSVC librarian: %MSVC_LIB% 1>&2
    exit /b 1
)
...
set "BUILD_DIR=%TEMP%\sqlite_static_%VENDOR_WINDOWS_ARCH%_%RANDOM%"
mkdir "%BUILD_DIR%" || exit /b 1
if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"

cl /nologo /O2 /D"SQLITE_ENABLE_RTREE" /c "%SOURCE_DIR%\sqlite3.c" /Fo"%BUILD_DIR%\sqlite3.obj"
if errorlevel 1 goto failed
"%MSVC_LIB%" /nologo /OUT:"%OUTPUT_DIR%\sqlite3.lib" "%BUILD_DIR%\sqlite3.obj"
if errorlevel 1 goto failed
