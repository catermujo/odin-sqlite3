...
LINK :: #config(SQLITE_LINK, "shared")

when LINK == "system" {
    when ODIN_OS == .Windows {
        #panic("vendor/sqlite does not support system linking on Windows")
    } else {
    ...
    when ODIN_OS == .Windows {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../windows_x64/sqlite3_shared.lib"
            } else when LINK == "static" {
                LIB_PATH :: "../windows_x64/sqlite3.lib"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../windows_arm64/sqlite3_shared.lib"
            } else when LINK == "static" {
                LIB_PATH :: "../windows_arm64/sqlite3.lib"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else {
            #panic("vendor/sqlite supports Windows amd64 and arm64 only")
        }
    } else when ODIN_OS == .Darwin {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../darwin_x64/libsqlite3.dylib"
            } else when LINK == "static" {
                LIB_PATH :: "../darwin_x64/sqlite3.darwin.a"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../darwin_arm64/libsqlite3.dylib"
            } else when LINK == "static" {
                LIB_PATH :: "../darwin_arm64/sqlite3.darwin.a"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else {
            #panic("vendor/sqlite supports Darwin amd64 and arm64 only")
        }
    } else when ODIN_OS == .Linux {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../linux_x64/libsqlite3.so"
            } else when LINK == "static" {
                LIB_PATH :: "../linux_x64/sqlite3.linux.a"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../linux_arm64/libsqlite3.so"
            } else when LINK == "static" {
                LIB_PATH :: "../linux_arm64/sqlite3.linux.a"
            } else {
                #panic("vendor/sqlite supports shared, static, and system linking only")
            }
        } else {
            #panic("vendor/sqlite supports Linux amd64 and arm64 only")
        }
    } else {
        #panic("vendor/sqlite supports Windows, Darwin, and Linux only")
    }

    when LINK == "static" && ODIN_OS == .Linux {
        foreign import sqlite3 {LIB_PATH, "system:pthread", "system:dl", "system:m"}
    } else {
        foreign import sqlite3 {LIB_PATH}
    }
}
...
VFS :: struct {
    ...
    dl_close:         proc "c" (vfs: ^VFS, dl: rawptr),
    ...
    get_system_call:  proc "c" (vfs: ^VFS, name: cstring) -> rawptr,
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    get_table :: proc(conn: ^Sqlite3, sql: cstring, result: ^^cstring, n_row: ^i32, n_column: ^i32, errmsg: ^cstring) -> Status ---
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    mprintf :: proc(fmt: cstring, #c_vararg args: ..any) -> cstring ---
    vmprintf :: proc(fmt: cstring, args: libc.va_list) -> cstring ---
    snprintf :: proc(n: i32, fmt: cstring, #c_vararg args: ..any) -> cstring ---
    vsnprintf :: proc(n: i32, fmt: cstring, args: libc.va_list) -> cstring ---
    log :: proc(err: i32, fmt: cstring, #c_vararg args: ..any) ---
}
...
foreign sqlite3 {
    ...
    soft_heap_limit64 :: proc(size: i64) -> i64 ---
    hard_heap_limit64 :: proc(size: i64) -> i64 ---
}

@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    memory_alarm :: proc(callback: proc "c" (ctx: rawptr, used: i64, allocation: i32), ctx: rawptr, threshold: i64) -> Status ---
}

@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    aggregate_count :: proc(ctx: ^Context) -> i32 ---
    expired :: proc(stmt: ^Stmt) -> b32 ---
    transfer_bindings :: proc(from: ^Stmt, to: ^Stmt) -> Status ---
    global_recover :: proc() -> Status ---
    thread_cleanup :: proc() ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    set_authorizer :: proc(conn: ^Sqlite3, auth: proc "c" (ctx: rawptr, action: Auth_Action, arg1: cstring, arg2: cstring, arg3: cstring, arg4: cstring) -> Status, ctx: rawptr) -> Status ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    trace :: proc(conn: ^Sqlite3, callback: proc "c" (ctx: rawptr, sql: cstring), ctx: rawptr) -> rawptr ---
    profile :: proc(conn: ^Sqlite3, callback: proc "c" (ctx: rawptr, sql: cstring, elapsed: u64), ctx: rawptr) -> rawptr ---
}
...
foreign sqlite3 {
    ...
    open16 :: proc(filename: rawptr, out_conn: ^^Sqlite3) -> Status ---
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    prepare :: proc(conn: ^Sqlite3, sql: cstring, sql_size: i32, out_stmt: ^^Stmt, out_unused: ^cstring) -> Status ---
    prepare16 :: proc(conn: ^Sqlite3, sql: rawptr, sql_size: i32, out_stmt: ^^Stmt, out_unused: ^rawptr) -> Status ---
    ...
    bind_pointer :: proc(stmt: ^Stmt, n: i32, ptr: rawptr, type_name: cstring, destructor: Destructor) -> Status ---
...
when #config(SQLITE_ENABLE_NORMALIZE, false) {
    @(link_prefix = "sqlite3_", default_calling_convention = "c")
    foreign sqlite3 {
...
foreign sqlite3 {
    ...
    set_auxdata :: proc(ctx: ^Context, n: i32, data: rawptr, destructor: Destructor) ---
...
foreign sqlite3 {
    ...
    value_subtype :: proc(value: ^Value) -> u32 ---
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    create_collation :: proc(conn: ^Sqlite3, name: cstring, encoding: Encoding, ctx: rawptr, compare: proc "c" (ctx: rawptr, e1: Encoding, s1: rawptr, e2: Encoding, s2: rawptr) -> i32) -> Status ---
    create_collation16 :: proc(conn: ^Sqlite3, name: [^]u16, encoding: Encoding, ctx: rawptr, compare: proc "c" (ctx: rawptr, e1: Encoding, s1: rawptr, e2: Encoding, s2: rawptr) -> i32) -> Status ---
    create_collation_v2 :: proc(conn: ^Sqlite3, name: cstring, encoding: Encoding, ctx: rawptr, compare: proc "c" (ctx: rawptr, e1: Encoding, s1: rawptr, e2: Encoding, s2: rawptr) -> i32, destroy: Destructor) -> Status ---
    collation_needed :: proc(conn: ^Sqlite3, ctx: rawptr, callback: proc "c" (ctx: rawptr, conn: ^Sqlite3, enc: Encoding, name: cstring)) -> Status ---
    collation_needed16 :: proc(conn: ^Sqlite3, ctx: rawptr, callback: proc "c" (ctx: rawptr, conn: ^Sqlite3, enc: Encoding, name: rawptr)) -> Status ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    db_cacheflush :: proc(conn: ^Sqlite3) -> Status ---
    system_errno :: proc(conn: ^Sqlite3) -> i32 ---
}
...
Module :: struct {
    ...
    find_function: proc "c" (
        ...
        arg: i32,
        name: cstring,
        func: ^proc "c" (contxt: ^Context, n: i32, value: ^^Value),
        ctx: ^rawptr,
    ) -> Status,
    ...
    // The methods above are valid for version 3.
    integrity:     proc "c" (vtab: ^VTab, schema: cstring, table: cstring, flags: i32, err: ^cstring) -> Status,
...
foreign sqlite3 {
    ...
    overload_function :: proc(conn: ^Sqlite3, func_name: cstring, arg: i32) -> Status ---
}

VTab_Config_Op :: enum i32 {
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    vtab_config :: proc(conn: ^Sqlite3, op: VTab_Config_Op, #c_vararg args: ..any) -> Status ---
    vtab_on_conflict :: proc(conn: ^Sqlite3) -> i32 ---
    vtab_nochange :: proc(ctx: ^Context) -> b32 ---
    vtab_collation :: proc(info: ^Index_Info, constraint: i32) -> cstring ---
    vtab_distinct :: proc(info: ^Index_Info) -> i32 ---
    vtab_in :: proc(info: ^Index_Info, constraint: i32, handle: b32) -> b32 ---
    vtab_in_first :: proc(value: ^Value, out: ^^Value) -> Status ---
    vtab_in_next :: proc(value: ^Value, out: ^^Value) -> Status ---
    vtab_rhs_value :: proc(info: ^Index_Info, constraint: i32, out: ^^Value) -> Status ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    blob_open :: proc(conn: ^Sqlite3,
        db: cstring,
        table: cstring,
        column: cstring,
        rowid: i64,
        flags: b32, // top 10 most misnamed variable names in existence.
        out_blob: ^Blob,) -> Status ---
    blob_reopen :: proc(blob: ^Blob, rowid: i64) -> Status ---
    blob_close :: proc(blob: ^Blob) -> Status ---
    blob_bytes :: proc(blob: ^Blob) -> i32 ---
    blob_read :: proc(blob: ^Blob, buf: rawptr, read: i32, offset: i32) -> Status ---
    blob_write :: proc(blob: ^Blob, buf: rawptr, write: i32, offset: i32) -> Status ---
}
...
Scan_Status_Flags :: bit_set[enum {
    Complex = 0,
};u32]
...
foreign sqlite3 {
    ...
    stmt_scanstatus :: proc(stmt: ^Stmt, index: i32, op: Scan_Status_Op, out: rawptr) -> Status ---
    stmt_scanstatus_v2 :: proc(stmt: ^Stmt, index: i32, op: Scan_Status_Op, flags: Scan_Status_Flags, out: rawptr) -> Status ---
    stmt_scanstatus_reset :: proc(stmt: ^Stmt) ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    backup_init :: proc(dst: ^Sqlite3, dst_name: cstring, src: ^Sqlite3, src_name: cstring) -> ^Backup ---
    backup_step :: proc(b: ^Backup, page: i32) -> Status ---
    backup_finish :: proc(b: ^Backup) -> Status ---
    backup_remaining :: proc(b: ^Backup) -> i32 ---
    backup_pagecount :: proc(b: ^Backup) -> i32 ---
}


Notify_Proc :: proc "c" (p_arg: ^rawptr, n_arg: i32)

@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    unlock_notify :: proc(blocked: ^Sqlite3, notify_proc: Notify_Proc, arg: rawptr) -> Status ---
...
Wal_Hook_Proc :: proc "c" (ctx: rawptr, s: ^Sqlite3, db: cstring, n_pages: i32) -> i32
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    wal_hook :: proc(s: ^Sqlite3, hook: Wal_Hook_Proc, ctx: rawptr) -> rawptr ---
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    test_control :: proc(op: i32, #c_vararg args: ..any) -> Status ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    snapshot_get :: proc(conn: ^Sqlite3, schema: cstring, out: ^^Snapshot) -> Status ---
    snapshot_open :: proc(conn: ^Sqlite3, schema: cstring, snapshot: ^Snapshot) -> Status ---
    snapshot_free :: proc(snapshot: ^Snapshot) ---
    snapshot_cmp :: proc(left: ^Snapshot, right: ^Snapshot) -> i32 ---
    snapshot_recover :: proc(conn: ^Sqlite3, schema: cstring) -> Status ---
}

Serialize_Flags :: bit_set[enum {
    No_Copy = 0,
};u32]

Deserialize_Flags :: bit_set[enum {
    Free_On_Close = 0,
    Resizeable    = 1,
    Readonly      = 2,
};u32]
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    serialize :: proc(conn: ^Sqlite3, schema: cstring, size: ^i64, flags: Serialize_Flags) -> [^]u8 ---
    deserialize :: proc(conn: ^Sqlite3, schema: cstring, data: [^]u8, size: i64, buffer_size: i64, flags: Deserialize_Flags) -> Status ---
}
...
@(link_prefix = "sqlite3_", default_calling_convention = "c")
foreign sqlite3 {
    rtree_geometry_callback :: proc(conn: ^Sqlite3, name: cstring, callback: proc "c" (geometry: ^RTree_Geometry, n_param: i32, params: ^RTree_Dbl, within: ^i32) -> i32, ctx: rawptr) -> Status ---
    rtree_query_callback :: proc(conn: ^Sqlite3, name: cstring, callback: proc "c" (info: ^RTree_Query_Info) -> i32, ctx: rawptr, destructor: Destructor) -> Status ---
}
